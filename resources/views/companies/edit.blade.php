...
@section('content')
    <main id="main" class="pt-16 h-screen px-28">
        <h2 class="my-6 text-4xl font-semibold text-center font-poppins tracking-widest text-gray-700 dark:text-gray-200">
            <span class="text-primary-100 dark:text-orange">Edit </span> Companie
        </h2>
                        ...
                        <div class="bg-white dark:bg-gray-700 shadow rounded-lg p-6">
                            ...
                            <form action="{{url('update/'.$companie->id)}}" method="POST">
                                ...
                                <div class="mb-4">
                                    ...
                                    <input type="text" value="{{$companie->name}}" placeholder="Companie Name" name="name"
                                        class="border p-2 rounded w-full">
                                </div>
                                <div class="mb-4">
                                    <label for="message"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Title</label>
                                    <input type="text" value="{{$companie->title}}" placeholder="your title here" name="title"
                                        class="border p-2 rounded w-full">
                                </div>  
                                <div class="mb-4">
                                    <label for="message"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">address</label>
                                    <input type="text" value="{{$companie->address}}" placeholder="your address here" name="address"
                                        class="border p-2 rounded w-full">
                                </div>  
                                <div class="mb-4">
                                    <label for="message"
                                        class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Founded at</label>
                                    <input type="date" value="{{$companie->founded_at}}" name="founded_at"
                                        class="border p-2 rounded w-full">
                                </div>                                
